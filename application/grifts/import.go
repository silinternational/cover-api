...
func normalizePolicy(p *LegacyPolicy) error {
	if p.Type == "household" {
		...
		if p.HouseholdId == "" {
			randomNumber := domain.RandomString(20, "0123456789")
			p.HouseholdId = randomNumber
			p.Notes += fmt.Sprintf(" [autogenerated HouseholdID %s during import]", randomNumber)
			log.Printf("Policy[%s] HouseholdId is empty, using random number: %s", p.Id, randomNumber)
		}
	}
	...
	if p.Type == "ou" || p.Type == "corporate" {
		...
		if !p.EntityCode.Valid || p.EntityCode.String == "" {
			randomNumber := domain.RandomString(20, "0123456789")
			p.EntityCode = nulls.NewString(randomNumber)
			p.Notes += fmt.Sprintf(" [autogenerated EntityCode %s during migration]", randomNumber)
			log.Printf("Policy[%s] EntityCode is empty, using random number: %s", p.Id, randomNumber)
		}
		if p.CostCenter == "" {
			randomNumber := domain.RandomString(20, "0123456789")
			p.CostCenter = randomNumber
			p.Notes += fmt.Sprintf(" [autogenerated CostCenter %s during migration]", randomNumber)
			log.Printf("Policy[%s] CostCenter is empty, using random number: %s", p.Id, randomNumber)
		}
	}
